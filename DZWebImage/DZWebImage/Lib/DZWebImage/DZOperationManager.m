...
@interface DZOperationManager ()

@property (nonatomic, strong) NSMutableDictionary<NSString*, UIImage*>* imageCache;
@property (nonatomic, strong) NSMutableDictionary<NSString*, DZWebOperation*> * oprationCache;
@property (nonatomic, copy) NSString *preUrl;
@property (nonatomic, strong) NSOperationQueue *queue;

@end

@implementation DZOperationManager
...
+ (instancetype)sharedOperationManager
{
    ...
    dispatch_once(&onceToken, ^{
        instance = [[DZOperationManager alloc]init];
    });
    return instance;
}
...
- (void)downloadImageWithUrl:(NSString *)urlStr andFinishBlock:(finishBlock)block
{
    ...
    //创建自定义操作对象
    DZWebOperation *operation = [[DZWebOperation alloc]initWithUrlStr:urlStr andBlock:^(UIImage *image) {
        NSLog(@"网络加载");
        if (image) {
            //将图片放入图片缓冲池
            [self.imageCache setObject:image forKey:urlStr];
...
- (NSMutableDictionary<NSString *,DZWebOperation *> *)oprationCache
{
    if (!_oprationCache) {
